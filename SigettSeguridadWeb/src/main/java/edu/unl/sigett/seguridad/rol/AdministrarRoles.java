...
public class AdministrarRoles implements Serializable {
    ...
    @EJB(lookup = "java:global/SeguridadService/RolPermisoDao!org.jlmallas.seguridad.dao.RolPermisoDao")
    ...
    public String crear(Usuario usuario) {
        ...
        try {
            ...
            int tienePermiso = usuarioService.tienePermiso(usuario, "crear_rol");
            if (tienePermiso == 1) {
    ...
    public void guardarRolesPermiso(Rol rol) {
        for (RolPermiso rolPermiso : rolPermisosGrabar) {
            ...
            if (contienePermiso(rolPermisoDao.buscarPorRol(rol.getId()), rolPermiso) == false) {
                ...
                logDao.create(logDao.crearLog("RolPermiso", rolPermiso.getId() + "", "CREAR", "|Rol= " + rolPermiso.getRolId() + "|Permiso= " + rolPermiso.getPermisoId(), sessionUsuario.getUsuario()));
            }
        }
    }
    ...
    public void buscar(String criterio, Usuario usuario) {
        try {
            FacesContext facesContext = FacesContext.getCurrentInstance();
            ResourceBundle bundle = facesContext.getApplication().getResourceBundle(facesContext, "msg");
            int tienePermiso = usuarioService.tienePermiso(usuario, "buscar_rol");
            if (tienePermiso == 1) {
    ...
    public String editar(Rol rol, Usuario usuario) {
        ...
        try {
            FacesContext facesContext = FacesContext.getCurrentInstance();
            ResourceBundle bundle = facesContext.getApplication().getResourceBundle(facesContext, "msg");
            int tienePermiso = usuarioService.tienePermiso(sessionUsuario.getUsuario(), "editar_rol");
            if (tienePermiso == 1) {
    ...
    public void removerRolPermisos(Rol rol) {
        if (rol.getId() != null) {
            for (RolPermiso rolPermiso : rolPermisosRemovidos) {
                ...
                if (rp != null) {
                    logDao.create(logDao.crearLog("RolPermiso", rp.getId() + "", "ELIMINAR", "|Rol=" + rp.getRolId() + "|Permiso=" + rp.getPermisoId(), sessionUsuario.getUsuario()));
        ...
        try {
            ...
            if (rol.getId() == null) {
                ...
                if (tienePermiso == 1) {
                    ...
                    rolDao.create(rol);
                    logDao.create(logDao.crearLog("Rol", rol.getId() + "", "CREAR", "|Nombre= " + rol.getNombre(), usuario));
                ...
                int tienePermiso = usuarioService.tienePermiso(usuario, "editar_rol");
                if (tienePermiso == 1) {
                    List<RolPermiso> rolPermisos = rolPermisoDao.buscarPorRol(rol.getId());
                    rolPermisos = rol.getRolPermisoList();
                    rol.setRolPermisoList(new ArrayList<RolPermiso>());
                    rolDao.edit(rol);
                    logDao.create(logDao.crearLog("Rol", rol.getId() + "", "EDITAR", "|Nombre= " + rol.getNombre(), sessionUsuario.getUsuario()));
                    guardarRolesPermiso(rol);
